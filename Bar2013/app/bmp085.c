...
{
        BMP085_SETTINGS_DEVICE_ADDRESS = 0x77, // ?∞??NИ?µN?
...
int16_t bmp085_read_temperature(void)
{
    ...
    int32_t x2;
    delay_ms(20);
    // запуск преобразовани€
    mcu_i2c_write_two_byte(BMP085_ADDRESS, 0xF4, 0x2E);
 
    // ждем окончани€ преобразовани€
    while (!(GPIOA->ODR & GPIO_ODR_ODR6))
    {
    }
 delay_ms(50);
    // чтение результата преобразовани€
...
void bmp085_init(void)
{
    ...
    // пауза перед обменом
    delay_ms(5);
     // устанавливаем указатель на начальный адрес корректировочных €чеек
    mcu_i2c_write_byte(BMP085_SETTINGS_DEVICE_ADDRESS, 0xAA);
 
    // считываем значение корректировочных €чеек
